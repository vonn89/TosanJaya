...
public class KeuanganController {
    ...
    @FXML
    private ComboBox<String> tipeKasirCombo;
    @FXML
    private TextField saldoAwalField;
    @FXML
    private TextField saldoAkhirField;
            ...
            public List<Keuangan> call() throws Exception {
                try (Connection con = Koneksi.getConnection()) {
                    ...
                    return KeuanganDAO.getAllByTanggalAndTipeKeuangan(con,
                            tglPicker.getValue().toString(), tglPicker.getValue().toString(), "true", tipeKasirCombo.getSelectionModel().getSelectedItem());
                }
            }
        };
        task.setOnRunning((e) -> {
        ...
        try {
            ...
            controller.passwordField.setOnKeyPressed((KeyEvent keyEvent) -> {
                if (keyEvent.getCode() == KeyCode.ENTER) {
                    mainApp.closeDialog(child);
                    if (Function.verifikasi(controller.usernameField.getText(), controller.passwordField.getText(), "Batal Transaksi")) {
                        Task<String> task = new Task<String>() {
                            @Override
                            public String call() throws Exception {
                                try (Connection con = Koneksi.getConnection()) {
